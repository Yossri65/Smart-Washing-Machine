...
Std_ReturnType MSSP_I2C_Init(const mssp_i2c_t *i2c_obj){
    ...
    if(NULL == i2c_obj){
        ...
        /* MSSP I2C Slew RatI2C_SLEW_RATE_ENABLE_CFGe Control */
...
static inline void MSSP_I2C_Interrupt_Configurations  (const mssp_i2c_t *i2c_obj)
{
        ...
        if(High_Priority == i2c_obj->priority)
        {
            INTERRUPT_HighPriorityEnable();
            I2C_INTERRUPT_Priority_High();
            I2C_Bus_Col_INTERRUPT_Priority_High();
        }
        else if(Low_Priority == i2c_obj->priority)
        {
            INTERRUPT_LowPriorityEnable();
            I2C_INTERRUPT_Priority_Low();
            I2C_Bus_Col_INTERRUPT_Priority_Low();
        }
        else{/*Nothing*/}
#else
